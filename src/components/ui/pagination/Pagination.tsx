                        ...
                        href={createPageUrl(currentPage - 1)} aria-disabled="true"> <IoChevronBackOutline size={30} /> </Link></li>
                    {
                        allPages.map((page, index) => (
                            <li key={page + '_' + index} className={
                                ...
                                className={
                                    clsx(
                                        "page-link relative block py-1.5 px-3 rounded border-0  outline-none transition-all duration-300   focus:shadow-md",
                                        {
                                            'bg-blue-600 hover:bg-blue-700 shadow-md text-white hover:text-white':page === currentPage
                                        }
                                    )
                                }
                                href={createPageUrl(page)}>{page} <span className="visually-hidden"></span></Link></li>
                        ))
                    }
